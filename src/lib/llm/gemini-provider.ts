...
export class GeminiProvider implements LLMProvider {
  ...
  async analyzeDocument(pdfData: Uint8Array | string): Promise<DocumentAnalysis> {
    ...
    const prompt = `Analyze the following text from a document and extract information. Return ONLY a JSON object.
...
Infer page numbers from position in text if not explicit.`;
  ...
  async convertWindow(
    ...
    return {
      markdown: `[Window ${context.position.windowNumber} conversion not supported for Gemini - use page-by-page processing]`,
      lastParagraph: '',
      summary: '',
...
RULES:
1. ${context.expectedHeadings.length > 0 ? 'Use EXACTLY the heading levels specified' : 'Infer heading levels from formatting'}
